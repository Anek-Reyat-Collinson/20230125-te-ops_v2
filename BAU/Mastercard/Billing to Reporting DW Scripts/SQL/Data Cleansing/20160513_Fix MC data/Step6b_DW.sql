...
SET @NumRecs = (SELECT COUNT(*) FROM mastercard_dw.dbo.factTblCIF WHERE report_month = @report_month) 

IF @NumRecs > 0
	BEGIN
		--PRINT 'Data already exists in mastercard_dw.dbo.factTblCIF for this month'
		DELETE FROM mastercard_dw.dbo.factTblCIF WHERE report_month = @report_month
	END
...
BEGIN	
	...
	(
		...
		WHERE cf.report_month = @report_month
