...
----------------------------------- Query Configuration  -----------------------------------------
DECLARE @NumRecs INT
DECLARE @Startbillingmonth AS DATE
DECLARE @Endbillingmonth AS DATE
SET @Startbillingmonth = '2016-03-01'
SET @Endbillingmonth = '2016-03-31'
...
------------------- (Add) tblLKCIF -----------------
--set @NumRecs = (select count(*) from mastercard_db.dbo.tblLKCIF WHERE [report_month] BETWEEN @Startbillingmonth AND @Endbillingmonth)
----select @NumRecs
--IF @NumRecs > 0
--	BEGIN
--		--PRINT 'Data Exists for this month in Target: mastercard_db.dbo.tblLKCIF'
--		DELETE from mastercard_db.dbo.tblLKCIF WHERE [report_month] BETWEEN @Startbillingmonth AND @Endbillingmonth
--	END
...
--	WHERE [report_month] BETWEEN @Startbillingmonth AND @Endbillingmonth
--	SET IDENTITY_INSERT mastercard_db.dbo.tblLKCIF OFF
--END	

----------------- (Add) tblLKLoungeVisits -----------------
set @NumRecs = (select count(*) from mastercard_db.dbo.tblLKLoungeVisits WHERE [report_month] BETWEEN @Startbillingmonth AND @Endbillingmonth)
IF @NumRecs > 0
	BEGIN
		DELETE from mastercard_db.dbo.tblLKLoungeVisits WHERE [report_month] BETWEEN @Startbillingmonth AND @Endbillingmonth
	END
