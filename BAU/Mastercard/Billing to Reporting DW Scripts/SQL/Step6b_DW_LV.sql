...
** Run history   :	GM 17/03/2016 Ran successfully for month = 2016-02-01
					...
					AE 14/08/2017 Ran successfully for July data --duplication of data https://jira.ptgmis.com/browse/BIBAU-130
					VP 14/08/2017 Ran successfully for Aug data 
					AA 13/10/2017 Ran successfully for Sep data 
					AA 14/11/2017 Ran successfully for OCT data --re run of the below on 15/11/2017 https://jira.ptgmis.com/browse/BIBAU-447
...
--- Set report month
DECLARE @StartDate AS DATE = '2019-12-01'
...
IF @NumRecs > 0
	...
	END
		...
		SELECT
			...
			c.currency_id as currency_code_fk,
			sum(lv.FeePerMember) as member_fee,
			sum(lv.TotalGuestFee) as guest_fee,
			sum(lv.TotalFee)as total_fee_mg,
			sum(lv.TotalFreeGuests) as free_guests,
			sum(lv.FreeGuestsValue) as free_guest_value,
			sum(lv.TotalChargableGuests) as chargable_guests,	
...
IF @NumRecs > 0
	BEGIN;
		...
		ENABLE TRIGGER [dbo].[trg_AutoAudit_factTblLVFrequency] ON [dbo].factTblLVFrequency; 
	END
